...
async def test_borrow_book_requires_auth(client, test_book):
    ...
    response = await client.post("/borrowings", json={"book_id": test_book.id})
...
async def test_borrow_nonexistent_book(client, auth_headers):
    ...
    response = await client.post(
        "/borrowings", json={"book_id": 99999}, headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_list_borrowings_by_user_id(
    ...
    response = await client.get(
        f"/borrowings?user_id={test_user.id}", headers=admin_auth_headers
    )
...
@pytest.mark.asyncio
async def test_list_borrowings_by_book_id(
    client, admin_auth_headers, test_book, db_session
):
    ...
    response = await client.get(
        f"/borrowings?book_id={test_book.id}", headers=admin_auth_headers
    )
...
async def test_list_active_borrowings(client, admin_auth_headers, db_session):
    ...
    borrowing_repo = BorrowingRepository(db_session)
    await borrowing_repo.create(
        {"user_id": 1, "book_id": 1, "due_date": utcnow_naive() + timedelta(days=14)}
    )

    response = await client.get(
        "/borrowings?active_only=true", headers=admin_auth_headers
    )
...
async def test_get_borrowing_by_id(client, auth_headers, test_user, db_session):
    ...
    response = await client.get(f"/borrowings/{borrowing.id}", headers=auth_headers)
...
async def test_max_books_limit(client, auth_headers, db_session):
    ...
    response = await client.post(
        "/borrowings", json={"book_id": new_book.id}, headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_borrow_after_return(
    client, test_book, auth_headers, librarian_auth_headers, db_session
):
