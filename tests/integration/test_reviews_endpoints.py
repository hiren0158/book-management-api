...
async def test_create_review(client, test_book, auth_headers, db_session):
    ...
    response = await client.post(
        ...
        json={"rating": 5, "text": "Excellent book!"},
...
async def test_create_review_requires_auth(client, test_book):
    ...
    response = await client.post(
        f"/books/{test_book.id}/reviews", json={"rating": 5, "text": "Great!"}
    )
...
async def test_create_duplicate_review(client, test_book, auth_headers, db_session):
    ...
    await review_repo.create(
        {"user_id": 1, "book_id": test_book.id, "rating": 4, "text": "First review"}
    )

    # Try to create second review
    response = await client.post(
        f"/books/{test_book.id}/reviews",
        json={"rating": 5, "text": "Second review"},
        headers=auth_headers,
    )
...
async def test_get_user_reviews(client, auth_headers, test_user, test_book, db_session):
    ...
    response = await client.get(f"/users/{test_user.id}/reviews", headers=auth_headers)
...
@pytest.mark.asyncio
async def test_get_other_user_reviews_denied(
    client, auth_headers, admin_user, test_book, db_session
):
    ...
    response = await client.get(f"/users/{admin_user.id}/reviews", headers=auth_headers)

    assert response.status_code == 403


@pytest.mark.asyncio
async def test_get_other_user_reviews_as_admin(
    client, admin_auth_headers, test_user, test_book, db_session
):
    ...
    response = await client.get(
        f"/users/{test_user.id}/reviews", headers=admin_auth_headers
    )
...
@pytest.mark.asyncio
async def test_delete_other_user_review_denied(
    client, auth_headers, test_book, db_session
):
...
@pytest.mark.asyncio
async def test_create_review_only_member(
