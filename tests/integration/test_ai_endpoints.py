...
@pytest.mark.asyncio
async def test_recommend_books_member_allowed(client, auth_headers, test_user, test_book, db_session):
...
@pytest.mark.asyncio
async def test_recommend_books_with_genre_filter(client, auth_headers, test_user, db_session):
    ...
    response = await client.get(
        "/ai/books/recommend?limit=5&genre=Mystery",
        headers=auth_headers
    )
...
async def test_recommend_books_limit_validation(client, auth_headers):
    ...
    response = await client.get(
        "/ai/books/recommend?limit=100",  # Exceeds max of 20
        headers=auth_headers
    )
...
async def test_natural_language_search_public(client):
    ...
    response = await client.post(
        "/ai/books/search_nl",
        json={"query": "books by J.K. Rowling"}
    )
...
async def test_natural_language_search_pagination(client, test_book):
    ...
    response = await client.post(
        "/ai/books/search_nl?limit=5&cursor=test",
