...
async def test_update_book_member_denied(client, test_book, auth_headers):
    ...
    response = await client.put(
        f"/books/{test_book.id}", json={"title": "Updated"}, headers=auth_headers
    )
    assert response.status_code == 403
...
async def test_delete_book_librarian_denied(client, test_book, librarian_auth_headers):
    ...
    response = await client.delete(
        f"/books/{test_book.id}", headers=librarian_auth_headers
    )
    assert response.status_code == 403
...
async def test_delete_book_admin_allowed(client, test_book, admin_auth_headers):
    ...
    response = await client.delete(f"/books/{test_book.id}", headers=admin_auth_headers)
...
async def test_borrow_book_member_allowed(client, test_book, auth_headers):
    ...
    response = await client.post(
        "/borrowings", json={"book_id": test_book.id}, headers=auth_headers
    )
    assert response.status_code == 201
...
async def test_borrow_book_admin_denied(client, test_book, admin_auth_headers):
    ...
    response = await client.post(
        "/borrowings", json={"book_id": test_book.id}, headers=admin_auth_headers
    )
    assert response.status_code == 403
...
async def test_borrow_book_librarian_denied(client, test_book, librarian_auth_headers):
    ...
    response = await client.post(
        "/borrowings", json={"book_id": test_book.id}, headers=librarian_auth_headers
    )
    assert response.status_code == 403
...
async def test_return_book_member_denied(client, test_book, auth_headers, db_session):
    ...
    response = await client.patch(
        f"/borrowings/{borrowing.id}/return", headers=auth_headers
    )
    assert response.status_code == 403


@pytest.mark.asyncio
async def test_return_book_admin_allowed(
    client, test_book, admin_auth_headers, db_session
):
    ...
    response = await client.patch(
        f"/borrowings/{borrowing.id}/return", headers=admin_auth_headers
    )
    assert response.status_code == 200


@pytest.mark.asyncio
async def test_return_book_librarian_allowed(
    client, test_book, librarian_auth_headers, db_session
):
    ...
    response = await client.patch(
        f"/borrowings/{borrowing.id}/return", headers=librarian_auth_headers
    )
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_create_review_member_allowed(
    client, test_book, auth_headers, db_session
):
...
@pytest.mark.asyncio
async def test_create_review_librarian_denied(
    client, test_book, librarian_auth_headers
):
...
@pytest.mark.asyncio
async def test_delete_review_own_review_member_allowed(
    client, test_book, auth_headers, db_session
):
    ...
    review = await review_repo.create(
        {"user_id": 1, "book_id": test_book.id, "rating": 4, "text": "Good book"}
    )

    response = await client.delete(f"/reviews/{review.id}", headers=auth_headers)
    assert response.status_code == 204


@pytest.mark.asyncio
async def test_delete_review_other_review_member_denied(
    client, test_book, auth_headers, db_session
):
...
@pytest.mark.asyncio
async def test_delete_review_admin_allowed(
    client, test_book, admin_auth_headers, db_session
):
    ...
    response = await client.delete(f"/reviews/{review.id}", headers=admin_auth_headers)
    assert response.status_code == 204
...
async def test_delete_user_librarian_denied(client, test_user, librarian_auth_headers):
    ...
    response = await client.delete(
        f"/users/{test_user.id}", headers=librarian_auth_headers
    )
    assert response.status_code == 403
...
async def test_delete_user_admin_allowed(client, test_user, admin_auth_headers):
    ...
    response = await client.delete(f"/users/{test_user.id}", headers=admin_auth_headers)
    assert response.status_code == 204
