...
@pytest.mark.asyncio
async def test_get_user_other_profile_as_librarian(
    client, test_user, librarian_auth_headers
):
...
async def test_update_user_own_profile(client, test_user, auth_headers):
    ...
    response = await client.put(
        f"/users/{test_user.id}", json={"name": "Updated Name"}, headers=auth_headers
    )
...
async def test_update_user_other_profile_as_member(client, admin_user, auth_headers):
    ...
    response = await client.put(
        f"/users/{admin_user.id}", json={"name": "Hacked Name"}, headers=auth_headers
    )
