...
def main():
    ...
    response = requests.post(f"{BASE_URL}/books", json=book_payload, headers=admin_headers)
    ...
    response = requests.post(f"{BASE_URL}/borrowings", json={"book_id": book_id}, headers=user_headers)
    ...
    review_payload = {
        "rating": 5,
        "text": "Great test book!"
    }
    response = requests.post(f"{BASE_URL}/books/{book_id}/reviews", json=review_payload, headers=user_headers)
    check_response(response, 201)

    # 9. User: Return Book
    print_step("9. User: Returning Book")
    response = requests.patch(f"{BASE_URL}/borrowings/{borrowing_id}/return", headers=user_headers)
    check_response(response)

    # 10. Verify RBAC (User tries to create book)
    print_step("10. RBAC Test: User trying to create book (Should Fail)")
    response = requests.post(f"{BASE_URL}/books", json=book_payload, headers=user_headers)
