...
def main():
    ...
    requests.post(f"{BASE_URL}/auth/register", json={
        "email": ADMIN_EMAIL, "name": "Admin Test", "password": PASSWORD, "role_id": 2
    })
    ...
    # Librarian
    requests.post(f"{BASE_URL}/auth/register", json={
        "email": LIBRARIAN_EMAIL, "name": "Librarian Test", "password": PASSWORD, "role_id": 3
    })
    librarian_token = requests.post(f"{BASE_URL}/auth/login", json={
        "email": LIBRARIAN_EMAIL, "password": PASSWORD
    }).json()["access_token"]
    librarian_headers = {"Authorization": f"Bearer {librarian_token}"}
    
    # Member
    requests.post(f"{BASE_URL}/auth/register", json={
        "email": MEMBER_EMAIL, "name": "Member Test", "password": PASSWORD, "role_id": 1
    })
    member_token = requests.post(f"{BASE_URL}/auth/login", json={
    ...
    response = requests.post(f"{BASE_URL}/books", json=book_payload, headers=librarian_headers)
    ...
    # Member can borrow
    response = requests.post(f"{BASE_URL}/borrowings", json={"book_id": book_id}, headers=member_headers)
    borrowing = check_response(response, 201)
    borrowing_id = borrowing["id"]
    print(f"✅ Member borrowed book, borrowing ID: {borrowing_id}")
    
    # Librarian CANNOT borrow
    response = requests.post(f"{BASE_URL}/borrowings", json={"book_id": book_id}, headers=librarian_headers)
    if response.status_code == 403:
    ...
    response = requests.patch(f"{BASE_URL}/borrowings/{borrowing_id}/return", headers=member_headers)
    if response.status_code == 403:
    ...
    response = requests.patch(f"{BASE_URL}/borrowings/{borrowing_id}/return", headers=librarian_headers)
