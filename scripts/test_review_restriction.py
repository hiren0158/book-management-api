...
def test_review_restriction():
    ...
    response = requests.post(
        ...
        json={"email": "member_review_test@example.com", "password": "testpass123"},
    )
    ...
    response = requests.post(
        f"{BASE_URL}/auth/login",
        json={"email": "admin_review_test@example.com", "password": "testpass123"},
    )
    ...
    if (
        ...
        and "can only review books you have borrowed" in response.text.lower()
    ):
    ...
    response = requests.post(
        f"{BASE_URL}/books/{book_id}/reviews",
        json={"rating": 5, "text": "Great book! I borrowed it first."},
        headers=member_headers,
    )
    ...
    response = requests.post(
        f"{BASE_URL}/books/{book_id}/reviews",
        json={"rating": 4, "text": "Changed my mind"},
        headers=member_headers,
    )
