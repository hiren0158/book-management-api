...
async def upload_pdf(
    ...
    logger.info(
        f"[RAG Upload] Started for file: {file.filename} by user {current_user.id}"
    )
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"[RAG Upload] ✓ Successfully uploaded {result.chunk_count} chunks"
            )
            return result
            ...
            logger.error(
                f"[RAG Upload] RAG service upload failed, rolling back database record"
            )
...
async def ask_question(
    ...
    if payload.doc_id != 0:
        has_access = await rag_service.check_document_access(
            payload.doc_id, current_user
        )
        if not has_access:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    try:
        ...
        result = await rag_client.ask_question(
            question=question, top_k=payload.top_k, doc_ids=doc_ids_to_search
        )
