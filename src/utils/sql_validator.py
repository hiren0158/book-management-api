...
class SQLWhereValidator:
    ...
    DANGEROUS_KEYWORDS = {
        'DELETE', 'UPDATE', 'DROP', 'CREATE', 'ALTER', 'TRUNCATE', 'INSERT',
        'UNION', 'JOIN', 'EXEC', 'EXECUTE', 'DECLARE', 'GRANT', 'REVOKE',
        'COMMIT', 'ROLLBACK', 'SAVEPOINT', 'SET', 'BEGIN', 'END'
    }
    
    # Allowed column names for the books table
    ALLOWED_COLUMNS = {
        'title', 'author', 'genre', 'description', 'isbn', 'published_date'
    }
    ...
    def validate(cls, where_clause: str) -> Tuple[bool, str]:
        ...
        for word in words:
            ...
            if is_dangerous:
                return False, f"Suspicious keyword detected: '{word}' (similar to dangerous keyword: {matched_keyword})"
        ...
        potential_columns = re.findall(r'\b([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:ILIKE|NOT\s+ILIKE|=|!=|<|>|<=|>=)', 
                                        where_clause, re.IGNORECASE)
        
        for col in potential_columns:
            if col.lower() not in cls.ALLOWED_COLUMNS:
                return False, f"Column '{col}' is not allowed. Allowed columns: {', '.join(cls.ALLOWED_COLUMNS)}"
        ...
        operators_found = re.findall(r'\b(ILIKE|NOT\s+ILIKE)\b|([=!<>]+)', where_clause, re.IGNORECASE)
        for op_tuple in operators_found:
            op = op_tuple[0] if op_tuple[0] else op_tuple[1]
            if op and op.upper().strip() not in cls.ALLOWED_OPERATORS and op not in ['=', '!=', '<', '>', '<=', '>=']:
