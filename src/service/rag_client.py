...
class RagClient:
    ...
    async def health_check(self) -> dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=503, detail="RAG service is currently unavailable"
            )

    async def upload_pdf(self, file: UploadFile, doc_id: int) -> UploadResponse:
        ...
        try:
            ...
            data = {"doc_id": doc_id}  # Send as integer, not string
            ...
            logger.error(
                f"[RAG Client] HTTP error: {e.response.status_code} - {e.response.text}"
            )
            raise HTTPException(
    ...
    async def ask_question(
        self, question: str, top_k: int = 5, doc_ids: Optional[List[int]] = None
    ) -> AskResponse:
        ...
        """
        try:
            request_data = AskRequest(question=question, top_k=top_k, doc_ids=doc_ids)
            ...
            logger.error("[RAG Client] Request timed out")
            raise HTTPException(status_code=504, detail="RAG service request timed out")
        except Exception as e:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.delete(
                    f"{self.base_url}/documents/{document_id}", headers=self.headers
                )

                # Handle 404 gracefully - document might not exist in vector store
                if response.status_code == 404:
                    logger.warning(
                        f"[RAG Client] Document {document_id} not found in RAG service"
                    )
            ...
            logger.warning(
                "RAG service deletion failed, but continuing with database deletion"
            )
            return {
                "message": "RAG service deletion failed, but database record removed"
            }
