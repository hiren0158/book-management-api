...
class BookRepository(BaseRepository[Book]):
    ...
    async def search_books_fts(
        ...
        if query and is_postgres:
            ...
            title_trigram_score = func.similarity(
                ...
                func.lower(literal(normalized_query))
            )
            ...
            genre_trigram_score = func.similarity(
                func.lower(Book.genre),
                func.lower(literal(normalized_query))
            )
            isbn_trigram_score = func.similarity(
                func.lower(Book.isbn),
                func.lower(literal(normalized_query))
            )
            ...
            combined_rank = (fts_rank * 0.7 + trigram_score * 0.3).label("combined_rank")

            stmt = (
                select(Book, combined_rank)
                .where(
                    or_(
            ...
            if items:
                logger.info(f"[FTS+Trigram] Query: '{query}' | Found {len(items)} results")
                return items, next_cursor
            else:
                logger.info(f"[FTS+Trigram] Query: '{query}' | No results, falling back to ILIKE")
        ...
        if published_year:
            custom_filters.append(lambda s: s.where(extract('year', Book.published_date) == published_year))

        search_fields = ["title", "description", "author", "genre", "isbn"]

        logger.info(f"[ILIKE Fallback] Query: '{query}' | Filters: author={author}, genre={genre}, year={published_year}")
    ...
    async def search_books_ilike(
        ...
        if query:
            ...
            stop_words = {'a', 'an', 'the', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'any', 'me', 'my', 'find', 'show', 'get'}
            keywords = [
                word.strip() 
        ...
        logger.info(f"[AI ILIKE-only] Query: '{query}' | Keywords: {keywords if query else 'none'} | Filters: author={author}, genre={genre}, year={published_year} | Found: {len(items)}")
    ...
    async def search_books_with_where_clause(
        ...
        logger.info(f"[AI SQL WHERE] WHERE clause: '{where_clause}' | Found: {len(items)}")
